...
public class CompletwdWorkoutHistoryController : MonoBehaviour, PageController
{
    ...
    void PageController.onInit(Dictionary<string, object> data, Action<object> callback)
    {
        ...
        {
            case WeightUnit.kg:
                totalWeightText.text = historyWorkout.totalWeight.ToString("F2")+" kg";
                break;
            case WeightUnit.lbs:
                totalWeightText.text = userSessionManager.Instance.ConvertKgToLbs(historyWorkout.totalWeight).ToString("F2")+" lbs";
                break;
        }
